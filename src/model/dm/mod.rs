...
use self::prob::{RHOB_INDEX, RHO_DM_INDEX, R_INDEX, SIGMAZ_INDEX};
...
pub fn log_prob<'py>(
    ...
    if ndim == 31 {
        let array = prob::log_prob1(&theta, zdata, wdata, &locs, &scales, dz);
        Ok(array.into_pyarray(py))
    } else {
        let array = prob::log_prob2(&theta, zdata, wdata, &locs, &scales, dz);
        Ok(array.into_pyarray(py))
    }
}
