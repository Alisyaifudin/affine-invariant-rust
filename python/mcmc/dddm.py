...
def f(z: ndarray, u: ndarray, theta: ndarray) -> Tuple[ndarray, ndarray]:
    ...
    ----------
        ...
        theta (`ndarray[1]`): parameters of the model, compose of
            ...
            `theta[25]`: dark disk surface density at midplane [Msun/pc^2]
            `theta[26]`: dark disk scale height [pc]
            `theta[27]`: number density of dark matter halo near midplane [pc^-3]
            `theta[28]`: the radial term (related to Oort's constant) [Msun/pc^3]
            `theta[29]`: sun height from midplane [pc]
            `theta[30]`: the average vertical velocity of the sun [km/s]
            `theta[31]`: the dispersion of the vertical velocity stars analized [km/s]
            `theta[32]`: normalization constant for the dispersion velocity
        -- `optional` --
            `theta[33]`: the second dispersion velocity term [km/s]
            `theta[34]`: the second normalization constant for the dispersion velocity
    """
    val = af.dddm.f(z, u, theta)
    return val
...
def solve_potential(theta: ndarray, z_start: float, z_end: float, dz: float) -> ndarray:
    ...
    """
    val = af.dddm.solve_potential(theta, z_start, z_end, dz)
    return val
...
def potential(z: ndarray, theta: ndarray, dz=10.) -> ndarray:
    ...
    """
    val = af.dddm.potential(z, theta, dz)
    return val
...
def log_prob(theta: ndarray, zdata: Tuple[ndarray, ndarray, ndarray], wdata: Tuple[ndarray, ndarray, ndarray], locs: ndarray, scales: ndarray, dz=10.) -> ndarray:
    ...
    ----------
        ...
        dz (`Optional[float]`): the step size of the distance from the galactic plane [pc] (default: 10.)
    """
    val = af.dddm.log_prob(theta, zdata, wdata, locs, scales, dz)
    return val
...
def generate_p0(nwalkers: int, locs: ndarray, scales: ndarray, kind: int) -> ndarray:
    ...
    """
    val = af.dddm.generate_p0(nwalkers, locs, scales, kind=kind)
    return val
...
def run_mcmc(nsteps: int, nwalkers: int, p0: ndarray, zdata: Tuple[ndarray, ndarray, ndarray], wdata: Tuple[ndarray, ndarray, ndarray], locs: ndarray, scales: ndarray, parallel=False, dz=10., batch=2, verbose=False) -> ndarray:
    ...
    """
    val = af.dddm.mcmc(nsteps, nwalkers, p0, zdata, wdata, locs, scales, parallel=parallel, dz=dz, batch=batch, verbose=verbose)
    return val
...
def sample(nsteps: int, nwalkers: int, pos: ndarray, theta: ndarray, parallel=False, dz=1., batch=2, verbose=False) -> ndarray:
        ...
        ----------
            ...
            verbose (`Optional[bool]`): whether to print the acceptance rate (default: False)
        """
        val = af.dddm.sample(nsteps, nwalkers, pos, theta, parallel=parallel, dz=dz, batch=batch, verbose=verbose)
        return val