...
// use super::prob;
use super::prob::{
    LOG_A1_INDEX, LOG_A2_INDEX, LOG_NU0_INDEX, RHOB_INDEX, MU0_INDEX, R_INDEX, SIGMAW1_INDEX,
...
impl ode_solvers::System<State> for Gravity {
    ...
    fn system(&self, _z: Height, y: &State, dy: &mut State) {
        let u = (y[0], y[1]);
        (dy[0], dy[1]) = dfz(
            &u,
            &self.rhob,
...
pub fn dfz(
    ...
    (
        u.1.clone(),
        4. * PI * G * rho_tot(&u.0, rhob, sigmaz, mu0, r),
    )
}
...
pub fn solve(
    ...
    let mu0 = theta.slice(s![.., MU0_INDEX]).to_owned();
