...
p0 = np.load(join(current_dir, 'data', f'p0-{model_kind}-dddm.npy'))
...
print("running mcmc...")
t0 = time()
sampler = dddm.run_mcmc(steps, nwalkers, p0, zdata, wdata, locs, scales, dz=1, verbose=True)
print(time() - t0, "s")
print(sampler.shape)
np.save(join(current_dir, 'data', f'chain-({data_kind})-{add_name}{model_kind}-0-dddm.npy'), sampler)