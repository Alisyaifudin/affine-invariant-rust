...
p0 = np.load(join(current_dir, 'data', f'p0-{model_kind}-no.npy'))
...
print("running mcmc...")
t0 = time()
sampler = no.run_mcmc(steps, nwalkers, p0, zdata, wdata, locs, scales, dz=1, verbose=True)
print(time() - t0, "s")

np.save(join(current_dir, 'data', f'chain-({data_kind})-{add_name}{model_kind}-0-no.npy'), sampler)
# sleep 2 seconds
import time
