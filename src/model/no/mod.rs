...
use self::prob::{RHOB_INDEX, R_INDEX, SIGMAZ_INDEX};
...
pub fn log_prob<'py>(
    ...
    if ndim != 30 && ndim != 32 {
        Err(PyValueError::new_err(format!(
            "Invalid number of dimensions: {}, expected 30 or 32",
            ndim
        )))?
    }
...
pub fn mcmc<'py>(
    ...
    if nwalkers <= 30 {
        panic!("nwalkers must be greater than {} (ndims)", 30);
    }
    ...
    if ndim != 30 && ndim != 32 {
        Err(PyValueError::new_err("p0 must have dimension 30 or 32"))?
    }
    let locs = locs.as_array().to_owned();
    let scales = scales.as_array().to_owned();
...
pub fn sample<'py>(
    ...
    if theta_dim != 30 && theta_dim != 32 {
        Err(PyValueError::new_err("theta must have dimension 30 or 32"))?
    }
    let pos = pos.as_array().to_owned();
